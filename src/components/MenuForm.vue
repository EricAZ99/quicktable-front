...
/**
 * MenuForm
 * Formulaire d'ajout ou de modification d'un menu.
 * Si la prop `menu` est fournie, le formulaire est en mode édition.
 */
import { ref } from 'vue';
...
/** Menu à modifier. Si null, le formulaire est en mode ajout */
const props = defineProps({
    menu: { type: Object, default: null }
})

/** Champ nom du menu, pré-rempli si en mode édition */
const nom = ref(props.menu?.name ?? '')
/** Champ description du menu */
const description = ref(props.menu?.description ?? '')
/** Champ catégorie du menu */
const category = ref(props.menu?.category ?? '')
/** Champ prix du menu */
const price = ref(props.menu?.price ?? '')
/** Champ image du menu */
const image = ref('')
/** État actif/inactif du menu */
    ...
    <FormBackGround>
        ...
        <form class="mt-4 space-y-4" @submit.prevent="onSubmit">
            <div>
                ...
                <InputText id="nom" v-model="nom" type="text" autocomplete="text" placeholder="ex: Menu africain" />
            </div>
            <div>
                <label class="text-sm font-medium text-slate-700" for="description">Description</label>
                <InputText id="description" v-model="description" type="text" placeholder="ex: Menu spécial des africains" />
            </div>
            ...
            <div class="flex justify-start items-center gap-4">
                <label class="inline-flex items-center gap-3 select-none cursor-pointer">
                    ...
                    <span class="text-sm text-slate-700">Désactiver</span>
                </label>
            </div>
