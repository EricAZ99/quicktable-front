    ...
    <FormBackGround>
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-400">Détails du menu</p>
                </div>
            </div>
        ...
        <div class="flex-1 overflow-y-auto px-6 py-5 space-y-5">
            ...
            <div class="flex flex-wrap gap-2">
                <span :class="menu.active ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-500 border-slate-200'"
                    class="inline-flex items-center gap-1.5 border rounded-full px-3 py-1 text-xs font-medium">
                    <span class="w-1.5 h-1.5 rounded-full inline-block"
                        :class="menu.active ? 'bg-emerald-400' : 'bg-slate-400'"></span>
            ...
            <div class="bg-slate-50 rounded-xl p-3 flex items-center gap-3">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-sm font-semibold text-slate-800 truncate">{{ menu.name || '—' }}</p>
                </div>
            </div>

            <!-- Description -->
            <div v-if="menu.description" class="bg-slate-50 rounded-xl p-4">
