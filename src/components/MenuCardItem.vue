...
<template>
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <div v-if="menus.length === 0" class="rounded-2xl border bg-white p-6 text-sm text-slate-600">
        ...
        <article v-for="menu in menus" :key="menu.id"
            ...
            <header class="flex items-start justify-between gap-3">
                <div class="flex items-center gap-3 min-w-0">
                    ...
                    <div class="min-w-0">
                        <h3 class="font-semibold text-slate-900 truncate">{{ menu.name }}</h3>
                        <p class="text-xs text-slate-500 truncate">{{ menu.description || '—' }}</p>
                    </div>
                </div>
            ...
            <div class="mt-3 flex items-center justify-end gap-1">
                <button type="button" @click="emit('view', menu)"
                ...
                <button type="button" @click="emit('edit', menu)"
                    class="p-1.5 rounded-lg text-slate-500 hover:bg-slate-100 transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="h-4 w-4" fill="currentColor">
                        <path d="M14.06,9L15,9.94L5.92,19H5V18.08L14.06,9M17.66,3C17.41,3 17.15,3.1 16.96,3.29L15.13,5.12L18.88,8.87L20.71,7.04C21.1,6.65 21.1,6 20.71,5.63L18.37,3.29C18.17,3.09 17.92,3 17.66,3M14.06,6.19L3,17.25V21H6.75L17.81,9.94L14.06,6.19Z" />
                    </svg>
                </button>
                <button type="button" @click="emit('delete', menu)"
